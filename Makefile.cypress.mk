...
e2e-accommodation e2e-alcohol-use e2e-drug-use e2e-employment-education e2e-finance e2e-health-wellbeing e2e-personal-relationships e2e-thinking-behaviours e2e-journeys e2e-misc:
	docker compose ${TEST_COMPOSE_FILES} -p ${PROJECT_NAME}-test run --rm --interactive=false -e CYPRESS_BASE_URL=${BASE_URL_CI} cypress ${CYPRESS_OPTIONS}
...
# List of all named spec patterns, so that a "catch-all" pattern can be created by excluding the named patterns
