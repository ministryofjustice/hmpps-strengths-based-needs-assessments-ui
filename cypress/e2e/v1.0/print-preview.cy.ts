...
describe('assessment print preview', () => {
  ...
  it('all answers are visible', () => {
    ...
    cy.getSummary('How many other people were involved with committing the current index offence(s)?')
      .getAnswer('None')
      .hasNoSecondaryAnswer()
    cy.getSummary('Does Sam recognise the impact on the victims or wider community?')
      .getAnswer('No')
      .hasNoSecondaryAnswer()
    ...
    cy.getSummary('Is there an escalation in seriousness from previous offending?')
      .getAnswer('No')
      .hasNoSecondaryAnswer()
    cy.getSummary(
