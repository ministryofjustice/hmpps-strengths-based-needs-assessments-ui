...
describe('Generate fixture for complete assessment', () => {
  ...
  it('completes the offence analysis section', () => {
    ...
    cy.visitSection(section)
    cy.getQuestion('Enter a brief description of the offence').enterText('¯\\_(ツ)_/¯')
    cy.getQuestion('When did the offence happen?').enterDate('23-01-1901')
    cy.getQuestion('Did the offence have any of the following elements?').getCheckbox('None').clickLabel()
    cy.getQuestion('Why did the offence happen?').enterText('¯\\_(ツ)_/¯')
    cy.getQuestion('What was Sam trying to gain from the offence?')
      .getCheckbox('Impressing friends/associates')
      .clickLabel()
    cy.getQuestion('Enter all victim details').enterText('¯\\_(ツ)_/¯')
