export default (summaryPage: string, analysisCompletePage: string, positionNumber: number) => {
  ...
  describe(questionWith('{SECTION_NAME}'), () => {
    ...
    it(`displays and validates the question`, () => {
      ...
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName))).hasValidationError('Select if there are any strengths or protective factors')
      cy.checkAccessibility()
    })

    it('"Give details" is optional when selecting "No"', () => {
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName))).getRadio('No').clickLabel()
      cy.markAsComplete()
