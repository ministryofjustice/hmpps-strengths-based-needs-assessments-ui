...
export default (summaryPage: string, analysisCompletePage: string, positionNumber: number, sectionName: string) => {
  ...
  const question = `${subjectPrefix} Sam's ${sectionName} linked to risk of reoffending?`
  const summaryQuestion = `${subjectPrefix} Sam's ${sectionName} linked to risk of reoffending?`
  ...
  describe(question, () => {
    ...
    it('"Give details" is optional when selecting "No"', () => {
      ...
      cy.getQuestion(question)
        ...
        .hasLimit(config.characterLimit.c1000)
        .hasNoValidationError()
