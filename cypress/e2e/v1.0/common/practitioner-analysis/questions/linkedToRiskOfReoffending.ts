export default (summaryPage: string, analysisCompletePage: string, positionNumber: number, sectionName: string) => {
  const question = `Is Sam’s ${sectionName} linked to risk of reoffending?`
  ...
  describe(question, () => {
    ...
    it(`displays and validates the question`, () => {
      ...
      cy.getQuestion(question).hasValidationError('Select if linked to risk of reoffending')
    ...
    it('"Give details" is required when selecting "Yes"', () => {
      ...
      cy.getAnalysisSummary(summaryQuestion)
        ...
        .hasSecondaryAnalysisAnswer('some  details\nnew line')
      cy.checkAccessibility()
      cy.getAnalysisSummary(summaryQuestion).clickChangeAnalysis()
      cy.assertStepUrlIs(summaryPage)
      cy.getQuestion(question).getRadio('Yes').isChecked()
      cy.getQuestion(question).getNextQuestion().hasText('some  details\nnew line')
    })
  })
}
