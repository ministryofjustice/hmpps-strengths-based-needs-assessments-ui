export default (summaryPage: string, analysisCompletePage: string, positionNumber: number) => {
  const questionWith = sectionName => `Is Sam’s ${sectionName} linked to risk of reoffending?`
  const summaryQuestion = 'Linked to risk of reoffending'
  ...
  describe(questionWith('{SECTION_NAME}'), () => {
    ...
    it(`displays and validates the question`, () => {
      ...
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName)))
        .hasValidationError('Select if linked to risk of reoffending')
      cy.checkAccessibility()
    })

    it('"Give details" is optional when selecting "No"', () => {
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName)))
        .getRadio('No').clickLabel()
      cy.markAsComplete()
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName)))
        .hasNoValidationError().getNextQuestion().hasTitle('Give details').hasNoValidationError()
    ...
    it('"Give details" is required when selecting "Yes"', () => {
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName))).getRadio('Yes').clickLabel()
      cy.markAsComplete()
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName)))
      ...
      cy.checkAccessibility()
      cy.markAsComplete()
      cy.getSectionName().then(sectionName => cy.getQuestion(questionWith(sectionName))).hasNoValidationError().getNextQuestion().hasTitle('Give details').hasNoValidationError()
      cy.visitStep(analysisCompletePage)
      cy.get('#tab_practitioner-analysis').click()
