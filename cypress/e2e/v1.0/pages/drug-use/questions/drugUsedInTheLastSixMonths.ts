...
export default (drug: string, injectable: boolean, stepUrl: string, summaryPage: string) => {
  ...
  describe(`${drug} - ${frequencyQuestion}`, () => {
    ...
    frequencies.forEach(frequency => {
      it(`summary page displays "${drug}" used "${frequency}"`, () => {
        ...
        cy.getDrugSummaryCard(drugName(drug), drugCardSection).hasCardItems(expectedDrugCardItems).getCardItem('How often').hasCardItemAnswers(frequency).clickChange()
        cy.assertStepUrlIs(stepUrl)
  ...
  describe(`${drug} - ${detailsQuestion}`, () => {
    it(`optional details field is displayed for "${drug}"`, () => {
      ...
      cy.visitStep(summaryPage)
      cy.checkAccessibility()
      cy.getDrugSummaryCard(drugName(drug), drugCardSection).hasCardItems(expectedDrugCardItems).getCardItem(detailsQuestion).hasCardItemAnswers('Some text').clickChange()
      cy.assertStepUrlIs(stepUrl)
