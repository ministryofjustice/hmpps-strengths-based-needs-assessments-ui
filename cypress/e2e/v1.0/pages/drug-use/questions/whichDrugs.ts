    ...
    drugs.forEach(({name: drug, injectable}) => {
      if (drug === 'Other') {
        it('displays and validates the conditional questions', () => {
          cy.getQuestion(question).getCheckbox(drug).hasHint(null).isNotChecked().hasConditionalQuestion(false).clickLabel()
          cy.getQuestion(question).getCheckbox(drug).getNthConditionalQuestion(0).hasHint('Add drug name').hasNoValidationError().hasText('')
          cy.getQuestion(question).getCheckbox(drug).getNthConditionalQuestion(1).hasHint(null).hasNoValidationError().hasRadios(lastUsedOptions)
          cy.saveAndContinue()

          cy.assertStepUrlIs(stepUrl)
          cy.getQuestion(question).getCheckbox(drug).getNthConditionalQuestion(0).hasValidationError(`Enter which other drug they've misused`)
          cy.getQuestion(question).getCheckbox(drug).getNthConditionalQuestion(1).hasValidationError(`Select when they last used this drug`)
        ...
        it('displays and validates the conditional question', () => {
          cy.getQuestion(question).getCheckbox(drug).hasHint(null).isNotChecked().hasConditionalQuestion(false).clickLabel()
          cy.getQuestion(question).getCheckbox(drug).getConditionalQuestion().hasHint(null).hasNoValidationError().hasRadios(lastUsedOptions)
          cy.saveAndContinue()

          cy.assertStepUrlIs(stepUrl)
          cy.getQuestion(question).getCheckbox(drug).getConditionalQuestion().hasValidationError(`Select when they last used this drug`)
      ...
      lastUsedOptions.forEach(lastUsed => {
        it(`summary page displays "${drug} - ${lastUsed}"`, () => {
          ...
          cy.visitStep(summaryPage)
          cy.getDrugSummaryCard(drugName(drug), drugCardSection).getCardItem('Last used').hasCardItemAnswers(lastUsedSummaryText).clickChange()
          cy.assertStepUrlIs(stepUrl)
          if (drug === 'Other') {
            cy.getQuestion(question).getCheckbox(drug).isChecked().getNthConditionalQuestion(0).hasText('Cake')
            cy.getQuestion(question).getCheckbox(drug).isChecked().getNthConditionalQuestion(1).getRadio(lastUsed).isChecked()
          } else {
            cy.getQuestion(question).getCheckbox(drug).isChecked().getConditionalQuestion().getRadio(lastUsed).isChecked()
          }
        })
      })
    })
