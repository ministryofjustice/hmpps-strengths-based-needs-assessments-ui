  ...
  describe(question, () => {
    ...
    drugOptions.forEach(option => {
      if (usedInTheLastSixMonths) {
        ...
        it(`conditional field "${conditionalQuestion}" is displayed for "${option}"`, () => {
          ...
          cy.getDrugSummaryCard(option, drugCardSection)
            .hasCardItems(expectedCardItems)
            .getCardItem('Injected')
            .hasCardItemAnswers('Yes', 'In the last 6 months')
            .clickChange()
          cy.assertStepUrlIs(stepUrl)
          ...
          cy.getQuestion(question)
            .getCheckbox(option)
            .getConditionalQuestion()
            .getCheckbox('More than 6 months ago')
            .isNotChecked()
            .clickLabel()
          cy.saveAndContinue()
          cy.visitStep(summaryPage)
          cy.getDrugSummaryCard(option, drugCardSection)
            .hasCardItems(expectedCardItems)
            .getCardItem('Injected')
            .hasCardItemAnswers('Yes', 'More than 6 months ago')
            .clickChange()
          cy.assertStepUrlIs(stepUrl)
          cy.assertQuestionUrl(question)
          cy.getQuestion(question)
          ...
          cy.getDrugSummaryCard(option, drugCardSection)
            .hasCardItems(expectedCardItems)
            .getCardItem('Injected')
            .hasCardItemAnswers('Yes', 'In the last 6 months', 'More than 6 months ago')
        })
      } else {
        it(`no conditional field is displayed for "${option}"`, () => {
          cy.getQuestion(question).getCheckbox(option).hasConditionalQuestion(false).clickLabel()

          cy.getQuestion(question).getCheckbox(option).hasConditionalQuestion(true).hasDisabledConditionalQuestion()
    ...
    it('selecting "None" deselects other options', () => {
      ...
      drugOptions.forEach(drug => {
        cy.visitStep(summaryPage)
        cy.getDrugSummaryCard(drug, drugCardSection)
          .hasCardItems(expectedCardItems)
          .getCardItem('Injected')
          .hasCardItemAnswers('No')
          .clickChange()
        cy.assertStepUrlIs(stepUrl)
        cy.assertQuestionUrl(question)
