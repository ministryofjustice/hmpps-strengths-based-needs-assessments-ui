describe('Generate fixture for complete assessment', () => {
  ...
  it('completes the finance section', () => {
    ...
    cy.visitSection(section)
    cy.getQuestion('Where does Sam currently get their money from?')
      .getCheckbox('No money')
      .clickLabel()
    cy.getQuestion('Does Sam have their own bank account?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('How good is Sam at managing their money?')
      ...
      .clickLabel()
    cy.getQuestion('Is Sam affected by gambling?')
      .getCheckbox('No')
      .clickLabel()
    cy.getQuestion('Is Sam affected by debt?')
      .getCheckbox('No')
      .clickLabel()
  ...
  it('completes the health and wellbeing section', () => {
    ...
    cy.getQuestion('Has Sam had a head injury or any illness affecting the brain?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('Does Sam have any neurodiverse conditions?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('Does Sam have any learning difficulties?')
      .getRadio('No difficulties')
      .clickLabel()
    cy.getQuestion('Is Sam able to cope with day-to-day life?')
      .getRadio('Not able to cope')
      .clickLabel()
    cy.getQuestion("What is Sam's attitude towards themselves?")
      .getRadio('Positive and reasonably happy')
      .clickLabel()
    cy.getQuestion('Has Sam ever self-harmed?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('Has Sam ever attempted suicide or had suicidal thoughts?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('How optimistic is Sam about their future?')
  ...
  it('completes the personal relationships and community section', () => {
    ...
    cy.getQuestion('Is Sam happy with their current relationship status?')
      .getRadio('Happy and positive about their relationship status or their relationship is likely to act as a protective factor')
      .clickLabel()
    cy.getQuestion("What is Sam's history of intimate relationships?")
      ...
      .clickLabel()
    cy.getQuestion('Is Sam able to resolve any challenges in their intimate relationships?')
      .enterText("¯\\_(ツ)_/¯")
    cy.getQuestion("What is Sam's current relationship like with their family?")
      .getRadio('Unknown')
      .clickLabel()
    cy.getQuestion("What was Sam’s experience of their childhood?")
      .getRadio('Positive experience')
      .clickLabel()
    cy.getQuestion("Did Sam have any childhood behavioural problems?")
      .getRadio('No')
      .clickLabel()
  ...
  it('completes the thinking, behaviours and attitudes section', () => {
    ...
    cy.getQuestion('Is Sam resilient towards peer pressure or influence by criminal associates?')
      .getRadio('Has been peer pressured or influenced by criminal associates in the past but recognises the link to their offending')
      .clickLabel()
    ...
    cy.getQuestion("Does Sam show manipulative behaviour or a predatory lifestyle?")
      .getRadio("Some evidence that they show manipulative behaviour or act in a predatory way towards certain individuals")
      .clickLabel()
    cy.getQuestion('Are there any concerns that Sam is a risk of sexual harm?')
      .getRadio('No')
      .clickLabel()
    cy.saveAndContinue()
    ...
    cy.getQuestion('Does Sam act on impulse?')
      .getRadio('Sometimes acts on impulse which causes problems')
      .clickLabel()
    cy.getQuestion('Does Sam have a positive attitude towards any criminal justice staff they have come into contact with?')
  ...
  it('completes the offence analysis section', () => {
    ...
    cy.visitSection(section)
    cy.getQuestion('Enter a brief description of the offence')
      .enterText("¯\\_(ツ)_/¯")
    cy.getQuestion('When did the offence happen?')
      .enterDate('1901-01-23')
    cy.getQuestion('Did the offence have any of the following elements?')
      .getCheckbox('None')
      .clickLabel()
    cy.getQuestion('Why did the offence happen?')
      .enterText("¯\\_(ツ)_/¯")
    cy.getQuestion('What was Sam trying to gain from the offence?')
      .getCheckbox('Impressing friends/associates')
      .clickLabel()
    cy.getQuestion('Enter all victim details')
      .enterText("¯\\_(ツ)_/¯")
    cy.getQuestion('Does Sam recognise the impact or consequences on the victims or others and the wider community?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('Is the offence linked to risk of serious harm, risks to the individual or other risks?')
      .getRadio('No')
      .clickLabel()
    cy.getQuestion('What are the patterns of offending?')
      .enterText("¯\\_(ツ)_/¯")
