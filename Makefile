...
CYPRESS_CLIENT_ID ?= hmpps-strengths-and-needs-ui-client
CYPRESS_CLIENT_SECRET ?= clientsecret
CYPRESS_API_TOKEN = $$(docker compose exec hmpps-auth curl \
   --location 'http://hmpps-auth:9090/auth/oauth/token' \
   --user $(CYPRESS_CLIENT_ID):$(CYPRESS_CLIENT_SECRET) \
...
e2e: ## Run the end-to-end tests in the Cypress app. Override the default base URL with BASE_URL=...
	npm i
	CYPRESS_API_TOKEN=$(CYPRESS_API_TOKEN) \
...
e2e-ci: ## Run the end-to-end tests in a headless browser. Used in CI. Override the default base URL with BASE_URL_CI=...
	...
	docker compose ${TEST_COMPOSE_FILES} run --rm -e CYPRESS_BASE_URL=${BASE_URL_CI} -e CYPRESS_API_TOKEN=${CYPRESS_API_TOKEN} cypress
